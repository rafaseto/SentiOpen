...
def main():
    ...
    try:
        ...
        cursor.execute("ALTER TABLE issue_comments ADD COLUMN IF NOT EXISTS sentiment_deepseek_r1_8b TEXT")  # Adiciona a coluna 'sentimento_gpt' na tabela se não existir
 
        for res in resultados:  
            cursor.execute(  
                "UPDATE issue_comments SET sentiment_deepseek_r1_8b = %s WHERE comment_id = %s",  
                (res[1], res[0])  
            )
