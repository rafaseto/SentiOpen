...
def main():
    ...
    try:
        ...
        def analyze_sentiment_comment(context, comment, model):
            ...
            resposta = ollama.chat(model=model, messages=[{"role": "user", "content": prompt}])
        ...
        cursor.execute("ALTER TABLE issue_comments ADD COLUMN IF NOT EXISTS sentiment_deepseek_r1_14b TEXT")  # Adiciona a coluna 'sentimento_gpt' na tabela se não existir
 
        for res in resultados:  
            cursor.execute(  
                "UPDATE issue_comments SET sentiment_deepseek_r1_14b = %s WHERE comment_id = %s",  
                (res[1], res[0])  
            )
