...
def main():
    ...
    try:
        ...
        stats_file = "stats_deepseek_v3.json"
        ...
        cursor.execute("ALTER TABLE issue_comments_from_release ADD COLUMN IF NOT EXISTS sentiment_deepseek_v3 TEXT")  # Adiciona a coluna 'sentimento_gpt' na tabela se não existir
  
        for res in resultados:  
            cursor.execute(  
                "UPDATE issue_comments_from_release SET sentiment_deepseek_v3 = %s WHERE comment_id = %s",  
                (res[1], res[0])  
            )
